...
import { FallbackMascotRenderer } from "./fallbackMascot";
...
export class MascotRenderer {
  ...
  constructor(
    ...
    this.#canvas.setAttribute("aria-label", "Rive mascot tracking preview");
  ...
  async initialize(): Promise<void> {
    this.#onStatus("checking", `Checking ${this.#assetPath}`);
    ...
    try {
      ...
      if (!response.ok) {
        this.#showFallback(
          "fallback-missing",
          `.riv not found (${response.status}); SVG fallback is active`,
        );
        return;
      }
      if (response.headers.get("content-type")?.includes("text/html")) {
        this.#showFallback(
          "fallback-missing",
          ".riv not found; the server returned its HTML fallback",
        );
        return;
      }
      ...
      this.#showFallback(
        "fallback-missing",
        ".riv could not be fetched; SVG fallback is active",
      );
      return;
    }
      ...
      this.#rive = new Rive({
        ...
        onLoad: () => {
          ...
          if (!this.#connectBindings()) {
            ...
            this.#showFallback(
              "fallback-invalid",
              ".riv loaded but required ViewModel properties are incomplete",
            );
            resolve();
            return;
          }
          this.#canvas.hidden = false;
          this.#fallback.element.hidden = true;
          this.#onStatus(
            "rive",
            `${ARTBOARD} · ${STATE_MACHINE} · ${VIEW_MODEL}`,
          );
          if (this.#lastOutput) {
            this.update(this.#lastOutput);
          }
          resolve();
        },
        onLoadError: () => {
          ...
          this.#showFallback(
            "fallback-invalid",
            ".riv failed runtime validation; SVG fallback is active",
          );
          resolve();
        },
      });
  ...
  #connectBindings(): boolean {
    ...
    const mode = instance.string("mode");
    const bodyYaw = instance.number("bodyYaw");
    const bodyPitch = instance.number("bodyPitch");
    const eyeX = instance.number("eyeX");
    const eyeY = instance.number("eyeY");
    const blink = instance.trigger("blink");
    const reset = instance.trigger("reset");
    const bodyScaleX = instance.number("_bodyScaleX");
    const faceOffsetX = instance.number("_faceOffsetX");
    const bodyScaleY = instance.number("_bodyScaleY");
    const faceOffsetY = instance.number("_faceOffsetY");
    const pupilLX = instance.number("_pupilLX");
    const pupilRX = instance.number("_pupilRX");
    const pupilLY = instance.number("_pupilLY");
    const pupilRY = instance.number("_pupilRY");
    const blinkScale = instance.number("_blinkScale");
    if (
      !mode ||
      !bodyYaw ||
      !bodyPitch ||
  ...
  #currentBlinkScale(nowMs: number): number {
    if (this.#blinkStartedMs === null) {
      return 1;
    }
    const elapsedMs = nowMs - this.#blinkStartedMs;
    if (elapsedMs <= 45) {
      return 1 - (elapsedMs / 45) * 0.92;
    }
    if (elapsedMs <= 70) {
      return 0.08;
    }
    if (elapsedMs <= 130) {
      return 0.08 + ((elapsedMs - 70) / 60) * 0.92;
    }
    this.#blinkStartedMs = null;
    return 1;
  }

  #showFallback(status: RendererStatus, detail: string): void {
    this.#canvas.hidden = true;
