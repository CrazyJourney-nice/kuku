...
- 一体化本地启动（主前端 + 本地视觉/voice）：

```bash
npm run dev
```

浏览器打开 <http://127.0.0.1:4174>。该命令同时启动主前端与 `127.0.0.1:8765` 本地算法服务；`npm run dev:local` 保留为兼容别名。若另一个 Kuku 开发项目已经启动视觉服务，本项目会复用它而不重复占用摄像头；最后一个开发进程退出时，脚本会关闭由它们共同启动的视觉服务。

- 如果 `4174` 已占用，可用 `KUKU_FRONTEND_PORT=4176 npm run dev` 覆盖前端端口。
- 仅启动前端（不含视觉/voice）：`npm run dev:frontend`
- 生产构建：`npm run build`
