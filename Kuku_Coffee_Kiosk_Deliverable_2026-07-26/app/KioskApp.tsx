...
function GuideStrip({
  ...
  targetScreen?: KioskContext["screen"];
  children: ReactNode;
}) {
  return (
    <div className="guide-strip">
      <div id={`kuku-slot-${targetScreen}`} className="kuku-stage-slot" />
      <p>{children}</p>
    </div>
  );
}
...
function OrderSummary({ context }: { context: KioskContext }) {
  if (!context.selectedDrinkId || !context.customization) return null;
  const drink = getDrinkById(context.selectedDrinkId);
  return (
...
function SubmittingScreen({ context }: { context: KioskContext }) {
  return (
    ...
    >
      <StepProgress current={3} />
      <GuideStrip screen="confirm" targetScreen="submitting">
        订单正在安全提交，请稍候。
      </GuideStrip>
      <div className="screen-scroll confirm-scroll">
        <header className="section-intro">
...
export function KioskApp() {
  ...
  useEffect(() => {
    ...
    startupRecoveryPending.current = Boolean(recovery);
  ...
  useEffect(() => {
    ...
    if (context.screen !== "impact") {
      startupRecoveryPending.current = false;
      return;
    }
    if (context.screen === "impact") {
      // On startup the initial impact screen renders before RESTORE_SNAPSHOT
      // takes effect. Preserve an existing in-flight snapshot through that
      // render; it may only be cleared after a real session returns home.
      if (startupRecoveryPending.current) return;
    ...
    <ErrorBoundary>
      ...
      >
        ...
        <ScreenTransitionDeck
          ...
          onTransitionComplete={() => send({ type: "TRANSITION_FINISHED" })}
        >
          {screen}
        </ScreenTransitionDeck>
