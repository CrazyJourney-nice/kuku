...
> 输入切片：`k1.png` → `k7.png`，其中 k1/k2 按用户最新要求交换页面位置  
...
本项目要开发一个可长期运行在咖啡售货机上的竖屏触控前端。用户从品牌公益首页进入，继续到欢迎页，再选择饮品、完成定制、确认订单、等待机器制作并取杯。
...
k1 与 k2 都有“开始点单”按钮。按用户最新指定的页面顺序，本规格定义：
...
- 点击 CTA 触发 `START_INTRO`，进入 k1 欢迎页。
- 该页不允许横向轮播，避免用户误以为必须浏览多屏。
- 图片资源未加载时显示暖色占位和信息卡，CTA 仍可用。
- 可配置首屏策略以便运营后续 A/B；跳过公益首页时应直接进入 k1 欢迎页。
...
- 所有饮品售罄：阻止进入定制，显示“当前饮品暂不可制作”，提供返回品牌公益首页。
...
- `completed` 后进入 k7。
- `cup_removed` 后延迟 800–1500ms 自动完成并回品牌公益首页。
- 仍保留“完成”按钮以处理传感器失效。

如果无取杯传感器：

- 用户点击“完成”即清空 session。
- 可配置 20–45 秒后自动回品牌公益首页。
...
| 当前状态 | 事件 | 条件 | 下一状态 |
|---|---|---|---|
| impact | START_INTRO | 设备可接单 | welcome |
| welcome | START_ORDER | 设备可接单 | drink |
...
| pickup | CUP_REMOVED | 传感器有效 | impact |
| pickup | FINISH_SESSION | 任意 | impact |
| drink/customize/confirm | IDLE_TIMEOUT | 无在途提交 | impact |
...
- impact：首页，不自动重置。
- welcome：30 秒回 impact 首页。
- drink/customize/confirm：45 秒无操作提示，60 秒重置。
- submitting/brewing：禁止无操作重置。
- pickup：20–45 秒后回 impact 首页，优先使用 cup removed。
...
应用启动
  ...
  → 无 snapshot：检查机器 ready，进入 impact 品牌公益首页
...
```text
正常路径：
impact → welcome → drink → customize → confirm
→ submit accepted → grinding → extracting → dispensing
→ completed → pickup → cup removed → impact
```
...
2. 在新项目中从零实现七屏流程的类型安全状态机：
   impact → welcome → drink → customize → confirm → submitting
   → brewing → pickup → impact。
...
```text
你负责实现 Kuku Coffee 七屏流程的 k2 公益首页与 k1 欢迎第二屏。
...
2. 实现 k2 的公益图片组合、年度统计、Kuku 感谢状态、气泡与 CTA。
3. 点击 k2 CTA 派发 START_INTRO，进入 k1。
4. 实现 k1 的 eyebrow、主标题、说明、Kuku、气泡与“开始点单”。
5. 点击 k1 CTA 派发 START_ORDER，进入 k3。
...
任务：
1. 实现统一 idle reset：impact 首页不重置，welcome 30s 回 impact，
